...
local Library do 
	Library = {
        ...
        Folders = {
            Directory = "Stainless",
            Configs = "Stainless/Configs",
            Assets = "Stainless/Assets",
			Sounds = "Stainless/Sounds",
        },
...
Library.SafeCall = function(self, Function, ...)
    ...
    if not Success then
        LocalPlayer:Kick("Stainless Callback Error: " .. tostring(Result))
        return false
    end
        ...
        local Items = { } do 
            ...
            Library:Connect(RunService.RenderStepped, function(Delta)
                ...
                if Now - LastTime >= 0.5 then
                    local FPS = MathFloor(FrameCount / (Now - LastTime) + 0.5)
                    FrameCount = 0
                    LastTime = Now

                    local TimeString = DateTime.now():FormatLocalTime("HH:mm:ss", "en-us")
                    Items["Info"].Instance.Text = "FPS: " .. FPS .. " | " .. TimeString
                end

                local Pulse = 0.6 + 0.15 * MathSin(tick() * 2)
                Items["Glow"].Instance.ImageTransparency = Pulse
            end)
        end

        function Watermark:SetText(Text)
            -- No longer needed as text is fixed and size is automatic
        end
    ...
    Library.KeybindList = function(self)
        ...
        function KeybindList:SetVisibility(Bool)
            if Bool then
                ...
                Items["KeybindList"]:Tween(nil, {Position = UDim2New(0, 20, 0.5, 0)})
            else
                local Tween = Items["KeybindList"]:Tween(nil, {Position = UDim2New(0, -260, 0.5, 0)})
                Library:Connect(Tween.Tween.Completed, function()
        ...
        function KeybindList:Add(Name, Key)
            ...
            NewKey:Tween(nil, {TextTransparency = 0.4})
